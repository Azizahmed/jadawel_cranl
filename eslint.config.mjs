...
export const createBaserowConfig = ({ extraSourceFiles = [] } = {}) => {
  return withNuxt([
    ...
    // ESLint 9 rejects an empty `files` array. The Jadawel OSS build has no
    // external plugin source files, so only add this override when it has a target.
    ...(extraSourceFiles.length
      ? [
          {
