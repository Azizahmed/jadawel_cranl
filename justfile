...
[group('0 - help')]
help:
    @echo "Jadawel Development - Getting Started"
    @echo "======================================"
    @echo ""
    ...
    @echo ""
    @echo "Documentation:"
    @echo "  README.md    - Project overview and dev stack setup"
    @echo "  AGENTS.md    - Repository guidelines, tech stack and test commands"
    @echo ""
...
_dev-start:
    ...
    echo "Starting Jadawel local development environment..."
    echo ""
...
_dc-dev-tmux:
    ...
    if ! docker ps --format '{{ '{{.Names}}' }}' | grep -q "^jadawel.*db"; then
        just dc-dev up -d
    fi
    # Kill any existing session
    if tmux has-session -t $SESSION 2>/dev/null; then
    ...
    new_tab() {
      ...
      if [[ "$OSTYPE" == "linux-gnu"* ]]; then
        ...
        if $PRINT_WARNING; then
            ...
            the Jadawel dev environment, please add support!"
            PRINT_WARNING=false
        fi
        print_manual_instructions "$COMMAND"
      fi
...
# WARNING: This clears ALL Docker builder cache, not just Jadawel!
[group('2 - docker-dev')]
[doc("Clear ALL Docker builder cache (not just Jadawel)")]
dc-cache-clear:
    @echo "WARNING: This will clear ALL Docker builder cache, not just Jadawel."
...
dc-fix-network:
    #!/usr/bin/env bash
    echo "Stopping and removing Jadawel containers with stale network references..."
    ...
    docker ps -aq --filter "name=jadawel" | xargs -r docker rm -f 2>/dev/null || true
    echo ""
...
build target="" tag="latest" *ARGS:
    ...
    if [[ "$MULTI" == "true" ]]; then
        ...
        if ! docker buildx inspect jadawel-multiarch >/dev/null 2>&1; then
            echo "Creating buildx builder 'jadawel-multiarch'..."
            docker buildx create --name jadawel-multiarch --use
        else
            docker buildx use jadawel-multiarch
        fi
    ...
    case "{{ target }}" in
        ...
        "web-frontend")
            ...
            NAME_ARG="jadawel/web-frontend:{{ tag }}"
            $BUILD_CMD "${BUILD_ARGS[@]}" $UID_GID_ARGS -f web-frontend/Dockerfile $TARGET_ARG -t $NAME_ARG .
            ;;
        "all-in-one")
            echo "Building backend (prod)..."
            $BUILD_CMD "${BUILD_ARGS[@]}" -f backend/Dockerfile --target prod -t jadawel/backend:{{ tag }} .
