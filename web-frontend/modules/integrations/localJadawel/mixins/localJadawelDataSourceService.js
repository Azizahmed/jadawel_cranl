...
export default {
  ...
  computed: {
    /**
     * Used by `LocalJadawelTableSelector` so that when read, we return the
  ...
  methods: {
    ...
    excludeTrashedFields(value) {
      const localJadawelFieldIds = this.tableFields.map(({ id }) => id)
      return value.filter(({ field }) => localJadawelFieldIds.includes(field))
    },
  },
}
