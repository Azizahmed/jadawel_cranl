<template>
  <form @submit.prevent>
    ...
    <p v-if="values.integration_id && !values.table_id">
      {{ $t('localJadawelUpsertRowServiceForm.noTableSelectedMessage') }}
    </p>
    <FieldMappingsForm
      v-if="!tableLoading"
    ...
    <Alert
      ...
      type="warning"
    >
      <p>{{ $t('localJadawelUpsertRowServiceForm.noWritableFields') }}</p>
    </Alert>
  </form>
</template>
...
import FieldMappingsForm from '@jadawel/modules/integrations/localJadawel/components/services/FieldMappingsForm'
...
export default {
  name: 'LocalJadawelUpsertRowServiceForm',
  components: {
    LocalJadawelServiceForm,
    FieldMappingsForm,
  },
  ...
  methods: {
    /**
     * When `LocalJadawelServiceForm` informs us that the table
    ...
    /**
     * When `LocalJadawelServiceForm` informs us that service specific
