<template>
  <FormRow class="margin-bottom-2">
    <FormGroup
      :label="$t('localJadawelTableSelector.databaseFieldLabel')"
      small-label
      required
    >
      <Dropdown
    ...
    <FormGroup
      :label="$t('localJadawelTableSelector.tableFieldLabel')"
      small-label
      required
    >
      <Dropdown
    ...
    <FormGroup
      ...
      :label="$t('localJadawelTableSelector.viewFieldLabel')"
      small-label
      required
    >
      <Dropdown
      ...
      >
        <DropdownItem
          :name="$t('localJadawelTableSelector.chooseNoView')"
          :value="null"
          >{{ $t('localJadawelTableSelector.chooseNoView') }}</DropdownItem
        >
        <DropdownItem
          v-for="view in views"
          :key="view.id"
...
<script>
export default {
  name: 'LocalJadawelTableSelector',
  props: {
    modelValue: {
      type: Number,
      required: false,
  ...
  methods: {
    getTableDescription(table) {
      if (table.is_two_way_data_sync) {
        return this.$t(
          'localJadawelTableSelector.twoWayDataSyncedTableDescription'
        )
      } else if (table.is_data_sync) {
        return this.$t(
          'localJadawelTableSelector.oneWayDataSyncedTableDescription'
        )
      }
      return null
    },
