<template>
  <div v-if="modelValue">
    <div v-if="modelValue.length === 0">
      <div class="filters__none">
        <div class="filters__none-title">
          {{ $t('localJadawelTableServiceConditionalForm.noFilterTitle') }}
        </div>
        <div class="filters__none-description">
          {{ $t('localJadawelTableServiceConditionalForm.noFilterText') }}
        </div>
      </div>
    </div>
    <ViewFieldConditionsForm
      ...
      :placeholder="
        $t('localJadawelTableServiceConditionalForm.textFilterInputPlaceholder')
      "
    ...
    >
      ...
      >
        <InjectedFormulaInput
          ...
          :placeholder="
            $t(
              'localJadawelTableServiceConditionalForm.formulaFilterInputPlaceholder'
            )
          "
          @input="
            updateFilter({
        ...
        <a
          ...
          :title="
            !filter.value_is_formula
              ? $t('localJadawelTableServiceConditionalForm.useFormulaForValue')
              : $t('localJadawelTableServiceConditionalForm.useDefaultForValue')
          "
    ...
    <div class="filters_footer">
      ...
      >
        {{ $t('localJadawelTableServiceConditionalForm.addFilter') }}
      </ButtonText>
    </div>
  </div>
</template>
...
export default {
  name: 'LocalJadawelTableServiceConditionalForm',
  components: {
    InjectedFormulaInput,
    ViewFieldConditionsForm,
  },
  ...
  methods: {
      ...
      try {
        ...
        if (field === undefined) {
          await this.$store.dispatch('toast/error', {
            title: this.$t(
              'localJadawelTableServiceConditionalForm.noCompatibleFilterTypesErrorTitle'
            ),
            message: this.$t(
              'localJadawelTableServiceConditionalForm.noCompatibleFilterTypesErrorMessage'
            ),
          })
        } else {
    ...
    updateFilter({ filter, values }) {
      const newFilters = this.modelValue.map((filterConf) => {
        if (filterConf.id === filter.id) {
          // Convert the formula value string into our Jadawel formula object.
    ...
    /**
     * Jadawel formula (i.e. non-database formulas) are objects, and within
