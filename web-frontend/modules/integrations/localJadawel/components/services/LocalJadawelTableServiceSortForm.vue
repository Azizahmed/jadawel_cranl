  ...
  <div v-if="modelValue">
    <div v-if="modelValue.length === 0" class="sortings__none">
      <div class="sortings__none-title">
        {{ $t('localJadawelTableServiceSortForm.noSortTitle') }}
      </div>
      <div class="sortings__none-description">
        {{ $t('localJadawelTableServiceSortForm.noSortText') }}
      </div>
    </div>
    <div
        ...
        <div v-if="sortMisconfigured(sort)">
          <div class="flex-grow-1">
            <p class="sortings__misconfigured-text">
              {{ $t('localJadawelTableServiceSortForm.sortMisconfigured') }}
            </p>
          </div>
        </div>
        ...
        <template v-else>
          <div class="sortings__description">
            <template v-if="index === 0">
              {{ $t('localJadawelTableServiceSortForm.sortBy') }}
            </template>
            <template v-if="index > 0">
              {{ $t('localJadawelTableServiceSortForm.thenBy') }}
            </template>
          </div>
    ...
    <template v-if="modelValue.length < availableFieldsLength && !disableSort">
      <div ref="addContextToggle">
        <ButtonText
          ...
          "
        >
          {{ $t('localJadawelTableServiceSortForm.addSort') }}
        </ButtonText>
      </div>
      <Context
        ref="context"
...
export default {
  name: 'LocalJadawelTableServiceSortForm',
  mixins: [context],
  props: {
    modelValue: {
      type: Array,
