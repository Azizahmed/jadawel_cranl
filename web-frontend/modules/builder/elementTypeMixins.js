...
export const CollectionElementTypeMixin = (Base) =>
  class extends Base {
     ...
     * 3. If a data source is found, `element` has a `schema_property`, and the integration is Jadawel, then:
     *   - "Repeat {schemaPropertyTitle} ({fieldTypeName})" is returned
     * 4. If a data source is found, `element` has a `schema_property`, and the integration isn't Jadawel, then:
    ...
    getDisplayName(element, { page, builder }) {
      ...
      if (collectionElement?.data_source_id) {
        ...
        if (element.schema_property) {
          ...
          // Only Local/Remote Jadawel table schemas will have `original_type`,
