# Vagrant Jadawel Ubuntu Install Test

> WARNING: this setup is currently broken. `bootstrap.sh` extracts its bash from
> `docs/guides/installation/old-install-on-ubuntu.md`, which no longer exists in
> this repository.

This folder contains a vagrant file and bootstrap.sh which together will start up a
local Ubuntu 18.04 virtual machine with Jadawel installed and working. To do this it
...
1. Install required vagrant plugins:
    1. `vagrant plugin install landrush`
       1. Used to setup a local dns server so you can visit Jadawel using a domain
    1. `vagrant plugin install vagrant-vbguest`
       1. So we can mount in your local Jadawel repo and use that to install Jadawel
    1. `vagrant vbguest`
...
1. `vagrant up`
    1. You might need to interactively enter your password for the landrush dns plugin
       to successfully add local dns entries on your host for Jadawel running inside the
       vm.
1. Wait a long time for everything to be provisioned
1. Once done visit [http://jadawel.vagrant.test](http://jadawel.vagrant.test) on your
   host machine to see the Jadawel running inside the vm.
