    ...
    if [ "$(id -u)" = '0' ]; then
      ...
      if [ -f "$ALREADY_SETUP_INDICATOR_FILE" ]; then
        ...
        if [ "$PGDATA_VERSION" != "$POSTGRES_VERSION" ]; then
          ...
          echo "See the 'Upgrading PostgreSQL database from a previous version' section of deploy/all-in-one/README.md for more information on how to upgrade your data directory with the upstream Baserow image ('baserow/baserow-pgautoupgrade:1.30.1') or how to keep running on the legacy PostgreSQL 11 image ('baserow/baserow-pg11:1.30.1')."
          echo
          exit 1
        fi
      else
