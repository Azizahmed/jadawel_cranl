...
def log_env_warnings():
    ...
    if settings.BASEROW_PUBLIC_URL:
        if settings.BASEROW_PUBLIC_URL.startswith("http://localhost"):
            logger.warning(
                "WARNING: Jadawel is configured to use a BASEROW_PUBLIC_URL of "
                f"{settings.BASEROW_PUBLIC_URL}. If you attempt to access Jadawel on "
                "any other hostname requests to the backend will fail as they will be "
                "from an unknown host. "
                "Please set BASEROW_PUBLIC_URL if you will be accessing Jadawel "
