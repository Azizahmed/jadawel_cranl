...
class JadawelBackupRunner:
    """
    A class which can backup or restore the database used by an installation of
    Jadawel. Under the hood it uses `pg_dump` and `pg_restore` commands.
    """
    ...
    def __init__(
        ...
        """
        Constructs a JadawelBackupRunner.
    ...
    def backup_jadawel(
        ...
        """
        Backs up an entire Database containing an installation of Jadawel.
        ...
        :raises InvalidJadawelBackupArchive: When the provided backup archive does not
            meet the expected jadawel backup format.
...
def _get_sorted_user_tables_names(conn) -> List[str]:
    """
    Queries the provided connection for the table and schema names of Jadawel's user
...
def _default_backup_location(database):
    ...
    return f"jadawel_backup_{database}_{now}.tar.gz"


def add_shared_postgres_command_args(parser):
    """
    The DB connection parameters are shared between the backup_jadawel and
    restore_jadawel commands.
    """
