...
class Command(BaseCommand):
    help = """
        Restores a Jadawel database back_up file generated by the Jadawel
        management command backup_jadawel. Only restoring into existing blank databases
        ...
        being updated, not connected to a running version of Jadawel for the
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            required=True,
            dest="file",
            help="Restores from the specified file produced by the jadawel "
            "management command backup_jadawel.",
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            runner.restore_jadawel(file, additional_args)
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully restored Jadawel to the database called: {database} "
                )
            )
