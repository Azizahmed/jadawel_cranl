...
from jadawel.contrib.database.constants import IMPORT_SERIALIZED_IMPORTING
from jadawel.core.auth_provider.registries import AuthenticationProviderTypeRegistry
from jadawel.core.exceptions import SubjectTypeNotExist
...
class Plugin(APIUrlsInstanceMixin, Instance):
    ...
    Example:
        ...
        from jadawel.core.registries import Plugin, plugin_registry
...
class PluginRegistry(APIUrlsRegistryMixin, Registry[Plugin]):
    ...
    abstraction made specifically for Jadawel. It allows a plugin developer to
...
class ApplicationType(
    ...
    Example:
        ...
        from jadawel.core.registries import ApplicationType, application_type_registry
    ...
    def export_safe_transaction_context(self, application: "Application") -> Atomic:
        ...
        jadawel.contrib.database.db.atomic.read_repeatable_single_database_atomic_transaction)
...
class ApplicationTypeRegistry(
    ...
    application is an abstraction made specifically for Jadawel. If added to the
...
class PermissionManagerTypeRegistry(Registry[PermissionManagerType]):
    ...
    Jadawel. A permission manager must then be listed in the
...
class ObjectScopeType(Instance, ModelInstanceMixin):
    """
    This type describe an object scope in Jadawel. This is useful if you need to know
...
class SubjectType(abc.ABC, Instance, ModelInstanceMixin):
    """
    This type describes a subject that exists in Jadawel. A subject is anything that
    ...
    def get_users_included_in_subject(self, subject) -> List["AbstractUser"]:
        ...
        And associated user is any user that receives permissions in Jadawel based
...
class SubjectTypeRegistry(Registry[SubjectType], ModelRegistryMixin[Any, SubjectType]):
    """
    This registry holds all the different subject types used across Jadawel.
    """
...
class OperationType(abc.ABC, Instance):
    ...
    def context_scope_name(cls) -> str:
        """
        An operation is executed on a context in Jadawel. For example the list_fields
