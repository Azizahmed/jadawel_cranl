...
from jadawel.core.registries import email_context_registry
...
class WorkspaceInvitationEmail(BaseEmailMessage):
    ...
    def get_subject(self):
        return _(
            "%(by)s invited you to %(workspace_name)s - جداول",
        ) % {
...
class NotificationsSummaryEmail(BaseEmailMessage):
    ...
    def get_subject(self):
        ...
        if count == 1:
            return _("You have 1 new notification - جداول")

        return _("You have %(count)d new notifications - جداول") % {"count": count}
