...
class Settings(models.Model):
    ...
    show_jadawel_help_request = models.BooleanField(
        default=True,
        help_text=(
...
class TrashEntry(models.Model):
    """
    A TrashEntry is a record indicating that another model in Jadawel has a trashed
    row. When a user deletes certain things in Jadawel they are not actually deleted
...
class DuplicateApplicationJob(
    ...
    original_application = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        help_text="The Jadawel application to duplicate.",
    )
    duplicated_application = models.OneToOneField(
        ...
        on_delete=models.SET_NULL,
        help_text="The duplicated Jadawel application.",
    )
