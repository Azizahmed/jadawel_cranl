...
from jadawel.contrib.database.action.scopes import DATABASE_ACTION_CONTEXT
...
from jadawel.core.utils import ChildProgressBuilder, Progress
...
class DeleteTableActionType(UndoableActionType):
    ...
    def do(cls, user: AbstractUser, table: Table):
        ...
        See jadawel.contrib.database.table.handler.TableHandler.delete_table
        for further details.
...
class OrderTableActionType(UndoableActionType):
    ...
    def do(cls, user: AbstractUser, database: Database, order: List[int]):
        ...
        See jadawel.contrib.database.table.handler.TableHandler.order_table
        for further details.
...
class UpdateTableActionType(UndoableActionType):
    ...
    def do(cls, user: AbstractUser, table: Table, name: str) -> Table:
        """
        Updates the table.
        See jadawel.contrib.database.table.handler.TableHandler.update_table
        for further details.
