...
from jadawel.cachalot_patch import cachalot_enabled
...
from jadawel.contrib.database.fields.fields import IgnoreMissingForeignKey
...
from jadawel.contrib.database.search.handler import (
    ALL_SEARCH_MODES,
    SearchHandler,
    SearchMode,
)
from jadawel.contrib.database.table.cache import (
...
from jadawel.core.db import MultiFieldPrefetchQuerysetMixin, specific_iterator
...
from jadawel.core.utils import are_kwargs_default, split_comma_separated_string
...
class GeneratedTableModel(HierarchicalModelMixin, models.Model):
    """
    Mixed into Model classes which have been generated by Jadawel.
    Can also be used to identify instances of generated jadawel models
    like `isinstance(possible_jadawel_model, GeneratedTableModel)`.
    """
    ...
    def info(cls):
        ...
        table = Table(
            title=f"{cls.jadawel_table.name} ({cls.jadawel_table.id})", box=box.ROUNDED
        )
    ...
    @classmethod
    def get_parent(cls):
        return cls.jadawel_table

    @classmethod
    def get_root(cls):
        return cls.jadawel_table.get_root()
...
class GeneratedModelAppsProxy:
    ...
    def __init__(self, jadawel_models=None, app_label=None):
        self.jadawel_models = jadawel_models or {}
        self.jadawel_app_label = app_label or "database_table"
    ...
    def get_models(self, *args, **kwargs):
        ...
        return apps.get_models(*args, **kwargs) + list(self.jadawel_models.values())
    ...
    def register_model(self, app_label, model):
        ...
        keep separate Django's model registry from Jadawel's generated models. In this
        ...
        with self._lock:
            ...
            if not hasattr(model, "_generated_table_model"):
                ...
                # jadawel models we can later use to resolve the pending operations.
                if not hasattr(self, "jadawel_models"):
                    self.jadawel_models = model._meta.auto_created.jadawel_models

            self.jadawel_models[model_name] = model
            self.do_all_pending_operations()
            self._clear_jadawel_models_cache()
            ...
            try:
                del apps.all_models[self.jadawel_app_label]
            except KeyError:
                pass

    def _clear_jadawel_models_cache(self):
        for model in self.jadawel_models.values():
    ...
    def do_all_pending_operations(self):
        ...
        for _ in range(max_iterations):
            ...
            pending_operations_for_app_label = [
                ...
                if app_label == self.jadawel_app_label
            ]
            for _, model_name in list(pending_operations_for_app_label):
                model = self.jadawel_models[model_name]
...
class Table(
    ...
    def _get_model(
        ...
        :param managed: Whether the created model should be managed by Django or not.
            ...
            generally Jadawel itself manages most aspects of returned generated models.
        :type managed: bool
        ...
        attrs = {
            ...
            "jadawel_table": self,
            "jadawel_table_id": self.id,
            "jadawel_models": apps.jadawel_models,
    ...
    def _fetch_and_generate_field_attrs(
        ...
        while len(fields) > 0:
            ...
            if filtered and add_dependencies:
                from jadawel.contrib.database.fields.dependencies.handler import (
            ...
            # are not displayed by jadawel anywhere.
...
class DuplicateTableJob(
    ...
    original_table = models.ForeignKey(
        ...
        help_text="The Jadawel table to duplicate.",
    )
    ...
    duplicated_table = models.OneToOneField(
        ...
        on_delete=models.SET_NULL,
        help_text="The duplicated Jadawel table.",
    )
