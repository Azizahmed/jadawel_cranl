...
def clear_generated_model_cache():
    print("Clearing Jadawel's internal generated model cache...")
    if hasattr(generated_models_cache, "delete_pattern"):
        ...
        raise ImproperlyConfigured(
            "Jadawel must be run with a redis cache outside of tests."
        )
...
def invalidate_table_in_model_cache(table_id: int):
    from jadawel.contrib.database.table.models import Table
    from jadawel.contrib.database.table.signals import table_schema_changed
