...
from jadawel.cachalot_patch import cachalot_disabled
from jadawel.core.db import IsolationLevel, sql, transaction_atomic
...
def read_repeatable_single_database_atomic_transaction(
    ...
    """
    If you want to safely read the contents of a Jadawel database inside of a single
    transaction and be guaranteed to see a single snapshot of the metadata and user
    data contained within the Jadawel db tables then use this atomic transaction context
    manager.
    ...
    2. This query runs that first transaction itself and intentionally locks all field
       ...
       We need to lock these rows as otherwise Jadawel's various endpoints can
...
def read_committed_single_table_transaction(
    ...
    """
    If you want to safely read the contents of a Jadawel table inside of a single
