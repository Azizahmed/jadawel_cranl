...
from jadawel.contrib.database.rows.handler import (
    GeneratedTableModelForUpdate,
    RowHandler,
)
from jadawel.contrib.database.rows.types import FileImportDict, UpdatedRowsData
...
class CreateRowActionType(UndoableActionType):
    ...
    def do(
        ...
        See the jadawel.contrib.database.rows.handler.RowHandler.create_row
...
class CreateRowsActionType(UndoableActionType):
    ...
    def do(
        ...
        belongs to the related workspace. It also calls the rows_created signal.
        See the jadawel.contrib.database.rows.handler.RowHandler.create_rows
        for more information.
...
class ImportRowsActionType(UndoableActionType):
    ...
    def do(
        ...
        See the jadawel.contrib.database.rows.handler.RowHandler.import_rows
        for more information.
...
class DeleteRowActionType(UndoableActionType):
    ...
    def do(
        ...
        See the jadawel.contrib.database.rows.handler.RowHandler.delete_row_by_id
        for more information.
...
class DeleteRowsActionType(UndoableActionType):
    ...
    def do(
        ...
        See the jadawel.contrib.database.rows.handler.RowHandler.delete_rows
        for more information.
...
class MoveRowActionType(UndoableActionType):
    ...
    def do(
        ...
        See the jadawel.contrib.database.rows.handler.RowHandler.move_row
        for more information.
...
class UpdateRowActionType(UndoableActionType):
    ...
    def do(
        ...
        See the jadawel.contrib.database.rows.handler.RowHandler.update_row
        for more information.
        ...
        :param view: Optionally provide view, if the row is updated in the view.
            This can result in different permissions checks.
        :param user_field_names: Whether or not the values are keyed by the internal
            Jadawel field names (field_1,field_2 etc) or by the user field names.
        :raises RowDoesNotExist: When the row with the provided id does not exist.
...
class UpdateRowsActionType(UndoableActionType):
    ...
    def do(
        ...
        See the jadawel.contrib.database.rows.handler.RowHandler.update_rows
        for more information.
        Undoing this action restores the original values.
        Redoing set the new values again.
