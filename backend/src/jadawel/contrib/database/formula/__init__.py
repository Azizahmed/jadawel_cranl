...
This module consists of 4 sub modules which are responsible for:
    1. parser: taking a raw Jadawel Formula string input, syntax checking it, parsing
       it and converting it to the internal Jadawel Expression format.
    2. ast: the definition of the internal Jadawel Expression abstract syntax tree (
       AST). Essentially a graph which can be used to represent a Jadawel Formula
       neatly and perform operations over.
    3. types: given an Jadawel Expression figures out the type of it. Or more
       specifically what postgres database column type should be used to store the
       result of the Jadawel Expression.
    4. expression_generator: takes a typed Jadawel Expression and generates a Django
       Expression object which calculates the result of executing the formula.

Also you can find the grammar definitions for the Jadawel Formula language may be found
in the
root folder formula_lang along with the scripts to generate the antlr4 parser found in
jadawel.contrib.formula.parser.generated .

The abstract syntax tree abstraction is used to decouple the specifics of the antlr4
parser from the specifics of turning a Jadawel formula into generated SQL. It lets us
...
different from the Jadawel Formula language, but generates a Jadawel Formula AST
