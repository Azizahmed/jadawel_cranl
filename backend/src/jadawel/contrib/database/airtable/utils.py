...
from jadawel.contrib.database.airtable.exceptions import AirtableSkipFilter
from jadawel.core.utils import get_value_at_path, remove_invalid_surrogate_characters
...
def quill_to_markdown(ops: list) -> str:
    ...
    to convert it in Jadawel compatible markdown. This is a simple, custom written
    function to convert it to Jadawel compatible markdown.
...
def airtable_date_filter_value_to_baserow(value: Optional[Union[dict, str]]) -> str:
    ...
    if isinstance(value, str):
        value = {
            "mode": "exactDate",
            "exactDate": value,
            "timeZone": "",  # it's okay to leave the timezone empty in Jadawel.
        }

    mode = value["mode"]
    if "exactDate" in value:
        # By default, Airtable adds the time, but that is not needed in Jadawel.
