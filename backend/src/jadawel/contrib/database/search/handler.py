...
from jadawel.contrib.database.fields.field_filters import FILTER_TYPE_OR, FilterBuilder
from jadawel.contrib.database.fields.models import Field
from jadawel.contrib.database.search.expressions import LocalisedSearchVector
from jadawel.contrib.database.search.models import (
    AbstractSearchValue,
    PendingSearchValueUpdate,
    get_search_indexes,
)
from jadawel.contrib.database.search.regexes import (
...
def _generate_search_table_model(
    ...
    from jadawel.contrib.database.table.models import GeneratedModelAppsProxy
...
class SearchHandler(
    ...
    def special_char_tokenizer(cls, expression: Expression) -> Func:
        ...
        Emails:
          ...
          2. jadawl.site
        URLs
          With input "https://jadawl.site/jobs/" this will result in tokens:
          1. https
          2. jadawl.site
          3. jobs
        Dates
    ...
    def escape_postgres_query(cls, text, per_token_wildcard: bool = False) -> str:
        ...
        :param per_token_wildcard: Determines whether we add the `:*` wildcard to
            ...
            but is problematic for Jadawel's frontend, so we only add the wildcard at
