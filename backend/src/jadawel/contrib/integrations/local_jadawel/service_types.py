...
from jadawel.contrib.database.api.utils import extract_field_ids_from_list
...
from jadawel.contrib.database.table.service import TableService
...
from jadawel.contrib.database.views.service import ViewService
from jadawel.contrib.database.views.view_aggregations import (
    DistributionViewAggregationType,
)
from jadawel.contrib.integrations.local_jadawel.api.serializers import (
    LocalJadawelTableServiceFieldMappingSerializer,
)
...
from jadawel.contrib.integrations.local_jadawel.mixins import (
    ...
    LocalJadawelTableServiceSortableMixin,
    LocalJadawelTableServiceSpecificRowMixin,
)
from jadawel.contrib.integrations.local_jadawel.models import (
    ...
    LocalJadawelTableService,
    LocalJadawelTableServiceFieldMapping,
    LocalJadawelUpsertRow,
    Service,
)
from jadawel.contrib.integrations.local_jadawel.utils import (
...
from jadawel.core.services.registries import (
    DispatchTypes,
    ListServiceTypeMixin,
    ServiceType,
    TriggerServiceTypeMixin,
)
from jadawel.core.services.types import (
    DispatchResult,
    FormulaToResolve,
    ServiceDict,
    ServiceSubClass,
...
class LocalJadawelServiceType(ServiceType):
    """
    The `ServiceType` for all `LocalJadawel` integration services.
    """

    integration_type = LocalJadawelIntegrationType.type
...
class LocalJadawelTableServiceType(LocalJadawelServiceType):
    """
    The `ServiceType` for `LocalJadawelTableService` subclasses.
    """
    ...
    serializer_field_overrides = {
        "table_id": serializers.IntegerField(
            required=False,
            allow_null=True,
            help_text="The id of the Jadawel table we want the data for.",
        ),
        "integration_id": serializers.IntegerField(
            required=False,
            allow_null=True,
            help_text="The id of the Jadawel integration we want the data for.",
        ),
    }

    # Three lists which represent the Jadawel `FieldType` which are not permitted
    ...
    def build_queryset(
        self,
        service: LocalJadawelTableService,
        table: "Table",
    ...
    def resolve_service_formulas(
        ...
        """
        A hook called before the `LocalJadawelTableServiceType` subclass dispatch
        calls. It ensures we check the service has a `Table` before execution.

        :param service: A `LocalJadawelTableService` instance.
    ...
    def import_property_name(
        ...
        """
        Responsible for taking a Local Jadawel property name (`Field.db_column`),
    ...
    def prepare_values(
        ...
        # added it in `LocalJadawelViewServiceType.prepare_values`.
    ...
    def generate_schema(
        ...
        :param service: A `LocalJadawelTableService` subclass.
    ...
    def get_schema_name(self, service: ServiceSubClass) -> str:
        """
        The default `LocalJadawelTableService` schema name.
    ...
    def get_used_field_names(
        self, service: LocalJadawelTableService, dispatch_context: DispatchContext
    ) -> Optional[List[str]]:
        """
    ...
    def get_table_model(
        self, service: LocalJadawelTableService
    ...
    def get_json_type_from_response_serializer_field(
        ...
        :param field: The Jadawel Field we want a type for.
        :param field_type: The Jadawel FieldType we want a type for.
...
class LocalJadawelViewServiceType(LocalJadawelTableServiceType):
    """
    The `ServiceType` for `LocalJadawelViewService` subclasses.
    """
        ...
        return {
            **super().serializer_field_overrides,
            "view_id": serializers.IntegerField(
                required=False,
                allow_null=True,
                help_text="The id of the Jadawel view we want the data for.",
            ),
        }

    class SerializedDict(LocalJadawelTableServiceType.SerializedDict):
        view_id: int
...
class LocalJadawelListRowsUserServiceType(
    ListServiceTypeMixin,
    LocalJadawelTableServiceSearchableMixin,
    LocalJadawelTableServiceFilterableMixin,
    LocalJadawelTableServiceSortableMixin,
    ...
    """
    This service gives access to a list of rows from the same Jadawel instance as the
    one hosting the application.
    """

    type = "local_jadawel_list_rows"
    model_class = LocalJadawelListRows
    dispatch_types = [
        DispatchTypes.DATA,
        DispatchTypes.ACTION,
    ]
    serializer_mixins = (
        LocalJadawelTableServiceFilterableMixin.mixin_serializer_mixins
        + LocalJadawelTableServiceSortableMixin.mixin_serializer_mixins
    )
    ...
    def serializer_field_names(self):
        return (
            ...
            + LocalJadawelTableServiceSortableMixin.mixin_serializer_field_names
    ...
    def serializer_field_overrides(self):
        return {
            **super().serializer_field_overrides,
            **LocalJadawelTableServiceSortableMixin.mixin_serializer_field_overrides,
    ...
    class SerializedDict(
        ...
        LocalJadawelTableServiceSortableMixin.SerializedDict,
    ...
    def get_max_result_limit(self, service: Service):
        return settings.INTEGRATION_LOCAL_JADAWEL_PAGE_SIZE_LIMIT
...
class LocalJadawelAggregateRowsUserServiceType(
    LocalJadawelTableServiceSearchableMixin,
    LocalJadawelTableServiceFilterableMixin,
    LocalJadawelViewServiceType,
):
    """
    This service gives access to aggregations over fields in a Jadawel table or view.
    """

    type = "local_jadawel_aggregate_rows"
    model_class = LocalJadawelAggregateRows
    dispatch_types = [
        DispatchTypes.DATA,
        DispatchTypes.ACTION,
    ]
    ...
    def get_schema_name(self, service: LocalJadawelAggregateRows) -> str:
        """
        The Local Jadawel aggregation schema name added to the `title` in
    ...
    def get_context_data_schema(
        self, service: LocalJadawelAggregateRows
    ) -> dict | None:
        return None
        ...
        return {
            ...
            **LocalJadawelTableServiceSearchableMixin.mixin_serializer_field_overrides,
            "field_id": serializers.IntegerField(
                required=False,
                allow_null=True,
                help_text="The id of the Jadawel field we want to aggregate on.",
            ),
        }
...
class LocalJadawelGetRowUserServiceType(
    LocalJadawelTableServiceSearchableMixin,
    LocalJadawelTableServiceFilterableMixin,
    LocalJadawelTableServiceSortableMixin,
    LocalJadawelTableServiceSpecificRowMixin,
    ...
    Jadawel instance as the one hosting the application.
    """

    type = "local_jadawel_get_row"
    model_class = LocalJadawelGetRow
    dispatch_types = [
        DispatchTypes.DATA,
        DispatchTypes.ACTION,
    ]
    ...
    def simple_formula_fields(self):
        return (
            super().simple_formula_fields
            + LocalJadawelTableServiceSearchableMixin.mixin_simple_formula_fields
            + LocalJadawelTableServiceSpecificRowMixin.mixin_simple_formula_fields
        )
    ...
    def allowed_fields(self):
        return (
            ...
            + LocalJadawelTableServiceFilterableMixin.mixin_allowed_fields
            + LocalJadawelTableServiceSearchableMixin.mixin_allowed_fields
            + LocalJadawelTableServiceSpecificRowMixin.mixin_allowed_fields
        )
    ...
    def serializer_field_names(self):
        return (
            ...
            + LocalJadawelTableServiceFilterableMixin.mixin_serializer_field_names
            + LocalJadawelTableServiceSearchableMixin.mixin_serializer_field_names
            + LocalJadawelTableServiceSpecificRowMixin.mixin_serializer_field_names
        )
    ...
    def serializer_field_overrides(self):
        return {
            ...
            **LocalJadawelTableServiceFilterableMixin.mixin_serializer_field_overrides,
            **LocalJadawelTableServiceSearchableMixin.mixin_serializer_field_overrides,
            **LocalJadawelTableServiceSpecificRowMixin.mixin_serializer_field_overrides,
        }
    ...
    class SerializedDict(
        ...
        LocalJadawelTableServiceSearchableMixin.SerializedDict,
        LocalJadawelTableServiceFilterableMixin.SerializedDict,
        LocalJadawelTableServiceSpecificRowMixin.SerializedDict,
    ):
        pass
...
class LocalJadawelUpsertRowServiceType(
    LocalJadawelTableServiceSpecificRowMixin, LocalJadawelTableServiceType
):
    """
    A `LocalJadawel` service type which will create or update rows in
    the service's target table when it is used in conjunction with
    workflow actions.
    """

    type = "local_jadawel_upsert_row"
    model_class = LocalJadawelUpsertRow
    ...
    def serializer_field_overrides(self):
        return {
            **super().serializer_field_overrides,
            "field_mappings": LocalJadawelTableServiceFieldMappingSerializer(
                many=True,
                required=False,
    ...
    class SerializedDict(
        LocalJadawelTableServiceType.SerializedDict,
        LocalJadawelTableServiceSpecificRowMixin.SerializedDict,
    ):
    ...
    def after_update(
        self,
        instance: LocalJadawelUpsertRow,
        values: Dict,
        ...
        if "field_mappings" in values and instance.table_id:
            ...
            LocalJadawelTableServiceFieldMapping.objects.bulk_create(
                bulk_field_mappings
            )
    ...
    def formula_generator(
        ...
        that implements LocalJadawelTableServiceType. E.g. a formula might be
    ...
    def formulas_to_resolve(
        self, service: LocalJadawelUpsertRow
    ...
    def dispatch_data(
        ...
        been provided, in this `LocalJadawelUpsertRow` service's table.
...
class LocalJadawelDeleteRowServiceType(
    LocalJadawelTableServiceSpecificRowMixin, LocalJadawelTableServiceType
):
    type = "local_jadawel_delete_row"
    model_class = LocalJadawelDeleteRow
    dispatch_types = [DispatchTypes.ACTION]
    ...
    def dispatch_data(
        self,
        service: LocalJadawelDeleteRow,
        resolved_values: Dict[str, Any],
        dispatch_context: DispatchContext,
    ) -> Dict[str, Any]:
        """
        Responsible for deleting a specific row if a row ID has been provided,
        in this `LocalJadawelDeleteRow` service's table.
...
class LocalJadawelRowsSignalServiceType(
    TriggerServiceTypeMixin, LocalJadawelTableServiceType
):
    # The signal that this service type is called by.
    signal: Signal = None

    # Local Jadawel signal services (e.g. rows created, updated, deleted)
    ...
    def import_path(self, path, id_mapping):
        """
        Import Local Jadawel signal service types paths.
        """
...
class LocalJadawelRowsCreatedServiceType(LocalJadawelRowsSignalServiceType):
    signal = rows_created
    type = "local_jadawel_rows_created"
    model_class = LocalJadawelRowsCreated


class LocalJadawelRowsUpdatedServiceType(LocalJadawelRowsSignalServiceType):
    signal = rows_updated
    type = "local_jadawel_rows_updated"
    model_class = LocalJadawelRowsUpdated


class LocalJadawelRowsDeletedServiceType(LocalJadawelRowsSignalServiceType):
    signal = rows_deleted
    type = "local_jadawel_rows_deleted"
    model_class = LocalJadawelRowsDeleted
