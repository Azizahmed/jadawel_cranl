...
from jadawel.contrib.database.fields.field_filters import FilterBuilder
...
from jadawel.contrib.integrations.local_jadawel.api.serializers import (
    LocalJadawelTableServiceFilterSerializerMixin,
    LocalJadawelTableServiceSortSerializerMixin,
)
...
from jadawel.core.services.utils import ServiceAdhocRefinements
from jadawel.core.utils import atomic_if_not_already
...
class LocalJadawelTableServiceFilterableMixin:
    """
    A mixin for LocalJadawel{Table,View}Service services so that when they dispatch,
    ...
    mixin_serializer_mixins = [LocalJadawelTableServiceFilterSerializerMixin]
    ...
    def enhance_queryset(self, queryset):
        return (
            ...
            .prefetch_related(
                Prefetch(
                    "service_filters",
                    queryset=LocalJadawelTableServiceFilter.objects.select_related(
                        "field"
                    ).all(),
                ),
            )
    ...
    def serialize_property(
        ...
        :param service: The LocalJadawelListRows service.
    ...
    def get_dispatch_filters(
        ...
        """
        Responsible for defining how the `LocalJadawel` services are filtered. To issue
        a `dispatch`, a `LocalJadawel` service must be pointing to a table.

        If we only have a `table` and no `view`, then we will query for, and apply,
        any `LocalJadawelTableServiceFilter` found for this service.

        If we also have a `view`, then we will query for, and apply, any `ViewFilter`
        found for this view.

        :param service: The `LocalJadawel` service we're dispatching.
        ...
        if isinstance(service, LocalJadawelViewService) and service.view_id:
    ...
    def formula_generator(
        self, service: "LocalJadawelTableServiceFilterableMixin"
    ) -> Generator[str | Instance, str, None]:
        """
        Generator that iterates over formula fields for LocalJadawel Services.
        ...
        for service_filter in service.service_filters_with_untrashed_fields:
            ...
            # Service types like LocalJadawelGetRow do not have a value attribute.
    ...
    def update_service_filters(
        self,
        service: Union[LocalJadawelGetRow, LocalJadawelListRows],
...
class LocalJadawelTableServiceSortableMixin:
    """
    A mixin for LocalJadawelTableService services so that when they dispatch, sortings
    applied to their service's table or view are applied to the queryset.
    """

    mixin_serializer_field_names = []
    mixin_serializer_field_overrides = {}
    mixin_serializer_mixins = [LocalJadawelTableServiceSortSerializerMixin]
    ...
    def enhance_queryset(self, queryset):
        return (
            ...
            .prefetch_related(
                Prefetch(
                    "service_sorts",
                    queryset=LocalJadawelTableServiceSort.objects.select_related(
                        "field"
                    ).all(),
                ),
            )
    ...
    def create_instance_from_serialized(
        ...
        **kwargs,
        ...
        # Create sortings
        LocalJadawelTableServiceSort.objects.bulk_create(
            [
                LocalJadawelTableServiceSort(
                    **service_sorting,
                    order=index,
    ...
    def get_dispatch_sorts(
        ...
        """
        Responsible for defining how the `LocalJadawel` services are sorted. To issue
        a `dispatch`, a `LocalJadawel` service must be pointing to a table.

        If we find any `LocalJadawelTableServiceSort` applied to this service, we will
        *only* sort on their `OrderBy` expressions.

        If we find no `LocalJadawelTableServiceSort`, then we will attempt to find any
...
class LocalJadawelTableServiceSearchableMixin:
    """
    A mixin for `LocalJadawelTable` service types so that when they dispatch,
...
class LocalJadawelTableServiceSpecificRowMixin:
