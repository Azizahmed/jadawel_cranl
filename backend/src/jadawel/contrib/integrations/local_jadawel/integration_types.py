...
from jadawel.api.user.serializers import SubjectUserSerializer
...
from jadawel.contrib.integrations.api.local_jadawel.serializers import (
    LocalJadawelContextDataSerializer,
)
...
class LocalJadawelIntegrationType(IntegrationType):
    type = "local_jadawel"
    model_class = LocalJadawelIntegration
    ...
    serializer_field_overrides = {
        "context_data": LocalJadawelContextDataSerializer(read_only=True),
    ...
    def after_import(
        self, user: AbstractUser, instance: LocalJadawelIntegration
    ) -> None:
        """
    ...
    def get_context_data(self, instance: LocalJadawelIntegration) -> Optional[Dict]:
        return {
            "databases": LocalJadawelIntegrationType.get_local_jadawel_databases(
                instance
            )
        }

    @staticmethod
    def get_local_jadawel_databases(integration: LocalJadawelIntegration) -> List:
        """
