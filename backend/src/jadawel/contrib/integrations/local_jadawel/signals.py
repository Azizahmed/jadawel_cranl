...
from jadawel.contrib.database.fields.signals import field_updated
...
def handle_local_jadawel_field_updated_changes(
    ...
    if kwargs.get("field_type_changed", False):
        ...
        if field.get_type().read_only:
            LocalJadawelTableServiceFieldMapping.objects.filter(field=field).delete()
        ...
        service_filters = LocalJadawelTableServiceFilter.objects.filter(
            field=old_field
        ).only("type", "id")
        ...
        LocalJadawelTableServiceFilter.objects.filter(
