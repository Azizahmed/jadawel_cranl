...
def check_lazy_loaded_libraries():
    ...
    This function checks sys.modules against settings.JADAWEL_LAZY_LOADED_LIBRARIES
    ...
    lazy_libs = getattr(settings, "JADAWEL_LAZY_LOADED_LIBRARIES", [])
    loaded_early = []
    ...
    if loaded_early:
        ...
        logger.warning(
            ...
            f"remove them from JADAWEL_LAZY_LOADED_LIBRARIES if they're legitimately "
            f"needed at startup. "
...
def log_env_warnings():
    ...
    if settings.JADAWEL_PUBLIC_URL:
        if settings.JADAWEL_PUBLIC_URL.startswith("http://localhost"):
            logger.warning(
                "WARNING: Jadawel is configured to use a JADAWEL_PUBLIC_URL of "
                f"{settings.JADAWEL_PUBLIC_URL}. If you attempt to access Jadawel on "
                "any other hostname requests to the backend will fail as they will be "
                "from an unknown host. "
                "Please set JADAWEL_PUBLIC_URL if you will be accessing Jadawel "
                f"from any other URL then {settings.JADAWEL_PUBLIC_URL}."
            )
    else:
        if "PUBLIC_BACKEND_URL" not in os.environ:
            logger.warning(
                "WARNING: Jadawel is configured to use a PUBLIC_BACKEND_URL of "
                f"{settings.PUBLIC_BACKEND_URL}. If you attempt to access Jadawel on any other "
                ...
                f"Jadawel from any other URL then {settings.PUBLIC_BACKEND_URL}."
            )
        if "PUBLIC_WEB_FRONTEND_URL" not in os.environ:
            logger.warning(
                "WARNING: Jadawel is configured to use a default PUBLIC_WEB_FRONTEND_URL "
                f"of {settings.PUBLIC_WEB_FRONTEND_URL}. Emails sent by Jadawel will use links "
                ...
                "the URL where users can access your Jadawel server."
            )
