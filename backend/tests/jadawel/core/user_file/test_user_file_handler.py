...
from jadawel.core.user_files.handler import MIME_TYPE_UNKNOWN, UserFileHandler
...
def test_upload_user_file_by_url(data_fixture, tmpdir):
    ...
    responses.add(
        responses.GET,
        "https://jadawl.site/not-found.pdf",
        status=404,
    )
    ...
    with pytest.raises(FileURLCouldNotBeReached):
        handler.upload_user_file_by_url(
            user, "https://jadawl.site/not-found.pdf", storage=storage
        )

    # Only the http and https protocol are supported.
    with pytest.raises(InvalidFileURLError):
        handler.upload_user_file_by_url(
            user, "ftp://jadawl.site/not-found.pdf", storage=storage
        )

    with freeze_time("2020-01-01 12:00"):
        user_file = handler.upload_user_file_by_url(
            user, "https://jadawl.site/test.txt", storage=storage
        )
...
def test_upload_user_file_by_url_with_querystring(data_fixture, tmpdir) -> None:
    ...
    remote_file = "https://jadawl.site/test.txt?utm_source=google&utm_medium=email&utm_campaign=fall"
...
def test_upload_user_file_by_url_with_image_without_extension_with_wrong_content_type(
    ...
    remote_file = "https://jadawl.site/image-without-url"
...
def test_upload_user_file_by_url_with_slash(data_fixture, tmpdir) -> None:
    ...
    remote_file = "https://jadawl.site/test.txt/"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("https://jadawl.site/", "https://jadawl.site/../.."),
        ("https://jadawl.site/", "https://jadawl.site/../."),
        ("https://jadawl.site/", "https://jadawl.site/.."),
        (
            "https://jadawl.site/",
...
def test_upload_user_file_by_url_with_invalid_content_type(
    ...
    responses.add(
        responses.GET,
        re.compile(r"https://jadawl.site.*"),
        body=b"Hello World",
        status=200,
