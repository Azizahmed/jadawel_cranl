...
from jadawel.core.models import Settings, Workspace
...
def test_specific_iterator_skip_missing_specific_objects(data_fixture):
    ...
    # skipped gracefully
    with patch("jadawel.core.db.logger") as mock_logger:
        specific_objects = list(
...
def test_get_approximate_row_count_returns_estimate_above_threshold():
    queryset = Workspace.objects.all()
    with patch("jadawel.core.db.APPROXIMATE_COUNT_THRESHOLD", 0):
        count = get_approximate_row_count(queryset)
