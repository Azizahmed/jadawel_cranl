...
from jadawel.contrib.database.fields.tasks import run_periodic_fields_updates
from jadawel.contrib.database.rows.handler import RowHandler
from jadawel.contrib.database.views.handler import ViewHandler, ViewSubscriptionHandler
...
@patch("jadawel.contrib.database.views.handler.ViewIndexingHandler")
...
def test_view_loaded_creates_last_modified_by_column(indexing_handler, data_fixture):
    ...
    with patch(
        "jadawel.contrib.database.table.tasks.setup_created_by_and_last_modified_by_column"
    ) as setup:
        view_loaded_create_indexes_and_columns(
...
def test_rows_enter_and_exit_view_are_called_when_rows_updated(data_fixture):
    ...
    with patch("jadawel.contrib.database.views.signals.rows_entered_view.send") as p:
    ...
    with patch("jadawel.contrib.database.views.signals.rows_exited_view.send") as p:
        row_handler.force_update_rows(
...
def test_rows_enter_and_exit_view_are_called_when_view_filters_change(
    ...
    with (
        patch(
            "jadawel.contrib.database.views.signals.rows_entered_view.send"
        ) as entered,
        patch("jadawel.contrib.database.views.signals.rows_exited_view.send") as exited,
    ):
...
def test_rows_enter_and_exit_view_with_periodic_fields_updates(data_fixture):
    ...
    with (
        patch("jadawel.contrib.database.views.signals.rows_entered_view.send") as p,
    ...
    with (
        patch("jadawel.contrib.database.views.signals.rows_exited_view.send") as p,
