...
from jadawel.contrib.database.webhooks.models import TableWebhook, TableWebhookCall
...
from jadawel.contrib.database.webhooks.registries import WebhookEventType
from jadawel.contrib.database.webhooks.tasks import (
    call_webhook,
    enqueue_webhook_task,
    schedule_next_task_in_queue,
)
from jadawel.core.models import WorkspaceUser
from jadawel.core.notifications.models import Notification
from jadawel.core.redis import WebhookRedisQueue
...
@patch("jadawel.contrib.database.webhooks.tasks.clear_webhook_queue")
...
def test_call_webhook_concurrent_task_moved_to_queue(data_fixture):
    from jadawel.contrib.database.webhooks.tasks import get_queue
...
@patch("jadawel.contrib.database.webhooks.tasks.RedisQueue", WebhookRedisQueue)
@patch("jadawel.contrib.database.webhooks.tasks.cache", MagicMock())
@patch("jadawel.contrib.database.webhooks.tasks.schedule_next_task_in_queue")
...
def test_webhook_with_paginated_payload(
    ...
    with (
        patch(
            "jadawel.contrib.database.webhooks.tasks.enqueue_webhook_task"
        ) as mock_enqueue,
        patch(
            "jadawel.contrib.database.webhooks.tasks.schedule_next_task_in_queue"
        ) as mock_schedule,
    ):
        call_webhook(
            webhook_id=webhook.id,
...
def test_call_webhook_payload_too_large_send_notification(
    ...
    with patch("jadawel.contrib.database.webhooks.tasks.call_webhook.delay") as mock:
        schedule_next_task_in_queue(webhook.id)
