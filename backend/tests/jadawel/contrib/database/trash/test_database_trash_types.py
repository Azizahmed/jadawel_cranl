...
@pytest.mark.django_db
@patch("jadawel.core.trash.signals.permanently_deleted.send")
...
def test_trash_and_restore_rows_in_batch(send_mock, data_fixture):
    ...
    with patch("jadawel.contrib.database.rows.signals.rows_created.send"):
        row1 = row_handler.create_row(
            user=user,
            table=customers_table,
            values={f"field_{customers_primary_field.id}": "Row A"},
...
def test_can_perm_delete_tables(
    ...
    patcher = patch("jadawel.core.models.TrashEntry.delete")
