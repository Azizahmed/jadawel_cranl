...
from jadawel.contrib.database.data_sync.exceptions import (
    PropertyNotFound,
    SyncDataSyncTableAlreadyRunning,
    UniquePrimaryPropertyNotFound,
)
from jadawel.contrib.database.data_sync.handler import DataSyncHandler
from jadawel.contrib.database.data_sync.ical_data_sync_type import (
    ICalCalendarDataSyncType,
    UIDICalCalendarDataSyncProperty,
)
from jadawel.contrib.database.data_sync.models import (
    DataSync,
    DataSyncSyncedProperty,
    ICalCalendarDataSync,
)
from jadawel.contrib.database.data_sync.registries import DataSyncTypeRegistry
from jadawel.contrib.database.fields.exceptions import CannotDeletePrimaryField
from jadawel.contrib.database.fields.handler import FieldHandler
from jadawel.contrib.database.fields.models import Field, LongTextField, TextField
...
def test_create_data_sync_table_no_unique_primary(data_fixture):
    ...
    with patch(
        "jadawel.contrib.database.data_sync.handler.data_sync_type_registry",
        new=registry,
    ):
        user = data_fixture.create_user()
        database = data_fixture.create_database_application(user=user)
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.table.signals.table_created.send")
def test_create_data_sync_table(send_mock, data_fixture):
    ...
    assert data_sync.ical_url == "https://jadawel.io"
...
@pytest.mark.django_db
@patch(
    "jadawel.contrib.database.data_sync.ical_data_sync_type.ICalCalendarDataSyncType.get_all_rows"
)
...
def test_trash_field_is_synced(data_fixture):
    ...
    data_sync.ical_url = "https://jadawel.io/ical2.ics"
    data_sync.save()
    fields = specific_iterator(data_sync.table.field_set.all().order_by("id"))
    FieldHandler().delete_field(user, fields[1])
