...
def test_create_row_with_disabled_webhook_events(api_client, data_fixture):
    ...
    with patch("jadawel.contrib.database.webhooks.registries.call_webhook.delay") as m:
        response = api_client.post(
...
def test_create_row_deadlock(api_client, data_fixture):
    ...
    with patch(
        "jadawel.contrib.database.rows.handler.RowHandler.force_create_row"
...
def test_update_row_deadlock(api_client, data_fixture):
    ...
    with patch(
        "jadawel.contrib.database.rows.handler.RowHandler.force_update_rows"
...
def test_move_row_deadlock(api_client, data_fixture):
    ...
    with patch(
        "jadawel.contrib.database.rows.handler.RowHandler.move_row"
    ) as mock_move_row:
...
def test_delete_row_deadlock(api_client, data_fixture):
    ...
    with patch(
        "jadawel.contrib.database.rows.handler.RowHandler.delete_row"
    ) as mock_delete_row:
            ...
            [
                {
                    ...
                    f"field_{url_field.id}": "https://jadawl.site",
        ...
        "results": [
            {
                ...
                "before": {
                    ...
                    f"field_{url_field.id}": "http://jadawl.site",
