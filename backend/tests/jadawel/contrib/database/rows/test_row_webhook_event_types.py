...
from jadawel.contrib.database.rows.webhook_event_types import RowsUpdatedEventType
from jadawel.contrib.database.webhooks.handler import WebhookHandler
from jadawel.contrib.database.webhooks.models import TableWebhook
from jadawel.contrib.database.webhooks.registries import webhook_event_type_registry
from jadawel.contrib.database.ws.rows.signals import serialize_rows_values
...
@patch("jadawel.contrib.database.webhooks.registries.call_webhook")
...
def test_rows_updated_event_type_can_trigger_webhooks_in_linked_tables_on_rows_created(
    ...
    with patch("jadawel.contrib.database.webhooks.registries.call_webhook.delay") as p:
...
def test_rows_updated_event_type_payload_for_linked_tables(data_fixture):
    ...
    with patch(
        "jadawel.contrib.database.webhooks.tasks.make_request_and_save_result",
    ) as p:
        (row_a1,) = (
            RowHandler()
            .force_create_rows(
