...
def test_urlfield_get_search_expression(data_fixture):
    ...
    row = RowHandler().force_create_row(
        user=user, table=table, values={f"field_{field.id}": "https://jadawl.site"}
    )
    model = table.get_model()
    qs = model.objects.all().pg_search("https://jadawl.site")
    assert qs.exists()
    matching_row = qs.get()
    assert matching_row.id == row.id
...
def test_emailfield_get_search_expression(data_fixture):
    ...
    row = RowHandler().force_create_row(
        user=user, table=table, values={f"field_{field.id}": "dev@jadawl.site"}
    )
    model = table.get_model()
    qs = model.objects.all().pg_search("dev@jadawl.site")
    assert qs.exists()
    matching_row = qs.get()
    assert matching_row.id == row.id
...
def test_last_modified_by_field_get_search_expression(data_fixture):
    user_adam = data_fixture.create_user(email="user1@jadawl.site", first_name="Adam")
    user_john_smith = data_fixture.create_user(
        email="user2@jadawl.site", first_name="John Smith"
    )
    user_mary_black = data_fixture.create_user(
        email="user3@jadawl.site", first_name="Mary Black"
    )
