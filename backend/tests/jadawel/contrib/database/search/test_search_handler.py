...
from jadawel.contrib.database.search.handler import SearchHandler, SearchMode
from jadawel.contrib.database.search.models import PendingSearchValueUpdate
...
def test_escape_query():
    ...
    assert SearchHandler.escape_query("https://jadawl.site") == "https jadawl site"
...
def test_update_rows_create_update_entries_for_all_updated_fields(data_fixture):
    ...
    with patch(
        "jadawel.contrib.database.search.handler.SearchHandler.process_search_data_updates"
    ) as mock:
        row1, row2, row3 = (
            RowHandler()
            .force_create_rows(
...
@pytest.mark.django_db()
@patch("jadawel.contrib.database.search.handler.SearchHandler.update_search_data")
...
def test_not_schedule_task_if_workspace_is_none(data_fixture):
    ...
    with patch(
        "jadawel.contrib.database.search.handler.SearchHandler.update_search_data"
    ) as mock:
...
@pytest.mark.django_db(transaction=True)
@patch("jadawel.contrib.database.search.tasks.update_search_data.apply_async")
