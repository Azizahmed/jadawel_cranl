...
def test_create_local_jadawel_integration_with_user(data_fixture):
...
@pytest.mark.django_db
def test_get_local_jadawel_databases_no_databases(data_fixture):
    user = data_fixture.create_user()
    integration = data_fixture.create_local_jadawel_integration(authorized_user=user)

    databases = LocalJadawelIntegrationType.get_local_jadawel_databases(integration)

    assert len(databases) == 0


@pytest.mark.django_db
def test_get_local_jadawel_databases(data_fixture):
    user = data_fixture.create_user()
...
@pytest.mark.django_db
def test_get_local_jadawel_databases_number_of_queries(
    ...
    with CaptureQueriesContext(connection) as queries:
        LocalJadawelIntegrationType.get_local_jadawel_databases(integration)
...
# to additional args.
# pytest -k "test_get_local_jadawel_databases_performance" -s --run-disabled-in-ci
# 4.500s for 30 x 999 x 100 table/views/fields on Intel(R) Core(TM) i9-9900K
# CPU @ 3.60GHz - # 7200 bogomips
def test_get_local_jadawel_databases_performance(data_fixture, api_client, profiler):
    ...
    print("------TYPE FUNCTION-------")
    with profiler(html_report_name="get_local_jadawel_databases_function"):
        LocalJadawelIntegrationType.get_local_jadawel_databases(integration)

    print("------SERIALIZATION-------")
    url = reverse("api:integrations:list", kwargs={"application_id": builder.id})
    with profiler(html_report_name="get_local_jadawel_databases_serialization"):
        api_client.get(
            url,
            format="json",
...
def test_after_import(data_fixture):
    """Test the LocalJadawelIntegrationType::after_import() method."""
