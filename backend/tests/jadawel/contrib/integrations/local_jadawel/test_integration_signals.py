...
from jadawel.contrib.database.fields.field_types import TextFieldType, UUIDFieldType
...
def test_local_jadawel_upsert_row_handle_field_mapping_field_changes(
    data_fixture,
):
...
@pytest.mark.django_db()
def test_local_jadawel_upsert_row_send_action_done_signal_when_creating_row(
    data_fixture,
):
    ...
    with patch("jadawel.core.action.signals.action_done.send") as send_mock:
...
@pytest.mark.django_db()
def test_local_jadawel_upsert_row_send_action_done_signal_when_updating_row(
    data_fixture,
):
    user, _ = data_fixture.create_user_and_token()
    workspace = data_fixture.create_workspace(user=user)
...
@pytest.mark.django_db()
def test_local_jadawel_upsert_row_send_action_done_signal_when_deleting_row(
    data_fixture,
):
    user, _ = data_fixture.create_user_and_token()
    workspace = data_fixture.create_workspace(user=user)
...
@pytest.mark.django_db()
def test_local_jadawel_service_filters_delete_when_field_type_changes_to_incompatible_type(
    data_fixture,
):
    ...
    link_row_filter = data_fixture.create_local_jadawel_table_service_filter(
        service=service, type="link_row_has", field=link_field, value="'1'", order=0
    )
    number_filter = data_fixture.create_local_jadawel_table_service_filter(
