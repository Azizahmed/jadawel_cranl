...
from jadawel.contrib.database.views.models import SORT_ORDER_DESC
...
def test_local_jadawel_table_service_refinement_manager(data_fixture):
...
@pytest.mark.django_db
def test_local_jadawel_table_service_field_mapping_manager(data_fixture):
    user = data_fixture.create_user()
    builder = data_fixture.create_builder_application(user=user)
    database = data_fixture.create_database_application(workspace=builder.workspace)
    integration = data_fixture.create_local_jadawel_integration(
    ...
    assert (
        LocalJadawelTableServiceFieldMapping.objects.filter(service=service).count()
        == 1
    )
    assert (
        LocalJadawelTableServiceFieldMapping.objects_and_trash.filter(
            service=service
        ).count()
        == 2
    )
