...
def test_local_jadawel_service_type_dispatch_types():
    local_jadawel_dispatch_types = {
        service_type.type: service_type.dispatch_types
        for service_type in service_type_registry.get_all()
        if isinstance(service_type, LocalJadawelServiceType)
    }
    assert local_jadawel_dispatch_types == {
        LocalJadawelGetRowUserServiceType.type: [
            DispatchTypes.DATA,
            DispatchTypes.ACTION,
        ],
        LocalJadawelListRowsUserServiceType.type: [
            DispatchTypes.DATA,
            DispatchTypes.ACTION,
        ],
        ...
        LocalJadawelUpsertRowServiceType.type: [
            DispatchTypes.ACTION,
        ],
        LocalJadawelDeleteRowServiceType.type: [
            DispatchTypes.ACTION,
        ],
        LocalJadawelRowsCreatedServiceType.type: [DispatchTypes.EVENT],
...
def test_local_jadawel_table_service_before_dispatch_validation_error(
    data_fixture,
):
    cls = LocalJadawelTableServiceType
...
@pytest.mark.django_db
def test_local_jadawel_table_service_generate_schema_with_no_table(data_fixture):
    user = data_fixture.create_user()
    ...
    get_row_service_type = LocalJadawelGetRowUserServiceType()
    ...
    list_rows_service_type = LocalJadawelListRowsUserServiceType()
...
@pytest.mark.django_db
def test_local_jadawel_table_service_generate_schema_with_interesting_test_table(
    data_fixture,
):
    ...
    expected_local_jadawel_table_service_schema_fields = {
...
def test_local_jadawel_service_type_get_schema_for_return_type():
    mock_service = Mock(id=123)
    cls = LocalJadawelServiceType
    cls.model_class = Mock()
...
def test_local_jadawel_table_service_type_schema_name():
    mock_service = Mock(table_id=123)
    assert (
        LocalJadawelGetRowUserServiceType().get_schema_name(mock_service)
        == "Table123Schema"
    )
    assert (
        LocalJadawelListRowsUserServiceType().get_schema_name(mock_service)
        == "Table123Schema"
    )


def test_local_jadawel_table_service_type_after_update_table_change_deletes_filters_and_sorts():
    mock_instance = Mock()
    ...
    service_type_cls = LocalJadawelListRowsUserServiceType
...
@pytest.mark.django_db
def test_local_jadawel_table_service_type_get_context_data(data_fixture):
    user = data_fixture.create_user()
    ...
    service_type = LocalJadawelGetRowUserServiceType()
...
@pytest.mark.django_db
def test_local_jadawel_table_service_type_get_context_data_schema(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    database = data_fixture.create_database_application(
        workspace=page.builder.workspace
...
@pytest.mark.django_db
def test_local_jadawel_view_service_type_prepare_values(data_fixture):
    user = data_fixture.create_user()
    ...
    service_type = LocalJadawelViewServiceType
    service_type.model_class = Mock()
    instance = data_fixture.create_local_jadawel_list_rows_service(
...
@pytest.fixture
def local_jadawel_get_context_data_fixture(data_fixture):
    """
    Fixture to help test LocalJadawelTableServiceType's get_context_data()
...
def test_local_jadawel_table_service_type_get_context_data_schema_excludes_fields(
    local_jadawel_get_context_data_fixture,
    allowed_fields_type,
    expect_none,
):
    """
    Test the LocalJadawelTableServiceType's get_context_data_schema() method.

    Ensure that when the optional allowed_fields list is supplied, any field
    not in the list are excluded from the schema.
    """

    field = local_jadawel_get_context_data_fixture["field"]
    service = local_jadawel_get_context_data_fixture["service"]
    service_type = local_jadawel_get_context_data_fixture["service_type"]
...
@pytest.mark.django_db
def test_local_jadawel_table_service_type_get_context_data_schema_excludes_fields_with_cache(
    local_jadawel_get_context_data_fixture,
):
    """
    Test the LocalJadawelTableServiceType's get_context_data_schema() method.
...
@pytest.mark.django_db
def test_local_jadawel_table_service_type_get_context_data_schema_cache_invalidation(
    local_jadawel_get_context_data_fixture,
):
    """
    Test the LocalJadawelTableServiceType's get_context_data_schema() method.
    ...
    service = local_jadawel_get_context_data_fixture["service"]
    service_type = local_jadawel_get_context_data_fixture["service_type"]
    table = local_jadawel_get_context_data_fixture["table"]
    user = local_jadawel_get_context_data_fixture["user"]
...
def test_local_jadawel_table_service_type_get_context_data_excludes_fields(
    local_jadawel_get_context_data_fixture,
    allowed_fields_id_indexes,
    expect_empty_dict,
):
    """
    Test the LocalJadawelTableServiceType's get_context_data() method.
...
@pytest.mark.django_db
def test_local_jadawel_table_service_type_get_context_data_excludes_fields_with_cache(
    local_jadawel_get_context_data_fixture,
):
    """
    Test the LocalJadawelTableServiceType's get_context_data() method.
...
@pytest.mark.django_db
def test_local_jadawel_table_service_type_get_context_data_cache_invalidation(
    local_jadawel_get_context_data_fixture,
):
    """
    Test the LocalJadawelTableServiceType's get_context_data() method.
...
@pytest.mark.django_db
def test_local_jadawel_table_agg_service_type_get_context_data_excludes_fields(
    data_fixture,
):
    user = data_fixture.create_user()
...
def test_local_jadawel_table_service_type_generate_schema_excludes_fields(
    local_jadawel_get_context_data_fixture,
    allowed_fields_id_indexes,
    expect_empty_dict,
):
    """
    Test the LocalJadawelTableServiceType's generate_schema() method.
...
@pytest.mark.django_db
def test_local_jadawel_table_service_type_generate_schema_excludes_fields_with_cache(
    local_jadawel_get_context_data_fixture,
):
    """
    Test the LocalJadawelTableServiceType's generate_schema() method.
...
@pytest.mark.django_db
def test_local_jadawel_table_service_type_generate_schema_cache_invalidation(
    local_jadawel_get_context_data_fixture,
):
    """
    Test the LocalJadawelTableServiceType's generate_schema() method.
...
@pytest.mark.django_db
def test_local_jadawel_agg_service_type_generate_schema_excludes_fields(data_fixture):
    user = data_fixture.create_user()
    table = data_fixture.create_database_table(user=user)
    field = data_fixture.create_number_field(table=table)
    service = data_fixture.create_local_jadawel_aggregate_rows_service(
...
def test_local_jadawel_rows_created_trigger_service_type_handler(data_fixture):
    mocked_on_event = Mock()
    user = data_fixture.create_user()
    service_type = service_type_registry.get(LocalJadawelRowsCreatedServiceType.type)
    service_type.on_event = mocked_on_event
    table = data_fixture.create_database_table(user=user)
    field = data_fixture.create_text_field(user, table=table)
    data_fixture.create_local_jadawel_rows_created_service(
        table=table,
    )
...
@pytest.mark.django_db(transaction=True)
def test_local_jadawel_rows_updated_trigger_service_type_handler(data_fixture):
    mocked_on_event = Mock()
    user = data_fixture.create_user()
    service_type = service_type_registry.get(LocalJadawelRowsUpdatedServiceType.type)
    service_type.on_event = mocked_on_event
    table = data_fixture.create_database_table(user=user)
    field = data_fixture.create_text_field(user, table=table)
    model = table.get_model()
    ...
    data_fixture.create_local_jadawel_rows_updated_service(
        table=table,
    )
...
@pytest.mark.django_db(transaction=True)
def test_local_jadawel_rows_deleted_trigger_service_type_handler(data_fixture):
    mocked_on_event = Mock()
    user = data_fixture.create_user()
    service_type = service_type_registry.get(LocalJadawelRowsDeletedServiceType.type)
    service_type.on_event = mocked_on_event
    table = data_fixture.create_database_table(user=user)
    model = table.get_model()
    row1 = model.objects.create()
