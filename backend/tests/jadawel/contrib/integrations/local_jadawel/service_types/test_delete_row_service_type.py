...
from jadawel.contrib.integrations.local_jadawel.models import LocalJadawelDeleteRow
from jadawel.contrib.integrations.local_jadawel.service_types import (
    LocalJadawelDeleteRowServiceType,
)
from jadawel.core.services.exceptions import DoesNotExist
...
def test_create_local_jadawel_delete_row_service(data_fixture):
    ...
    service_type = LocalJadawelDeleteRowServiceType()
    ...
    service: LocalJadawelDeleteRow = ServiceHandler().create_service(  # type: ignore
        service_type, **values
    )
...
@pytest.mark.django_db
def test_update_local_jadawel_delete_row_service(data_fixture):
    user = data_fixture.create_user()
    workspace = data_fixture.create_workspace(user=user)
    page = data_fixture.create_builder_page(user=user)
    database = data_fixture.create_database_application(workspace=workspace)
...
@pytest.mark.django_db
def test_local_jadawel_delete_row_service_dispatch_data_with_no_row_id(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_delete_row_service_dispatch_data_row_not_exist(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table = data_fixture.create_database_table(user=user)
    model = table.get_model()
...
@pytest.mark.django_db
def test_local_jadawel_delete_row_service_dispatch_data(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table = data_fixture.create_database_table(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_delete_row_service_dispatch_transform(data_fixture):
    service_type = LocalJadawelDeleteRowServiceType()
