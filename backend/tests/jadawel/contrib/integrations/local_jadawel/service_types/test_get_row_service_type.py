...
def test_create_local_jadawel_get_row_service(data_fixture):
...
@pytest.mark.django_db
def test_export_import_local_jadawel_get_row_service(data_fixture):
    user = data_fixture.create_user()
    path_params = [
    ...
    service: LocalJadawelGetRow = service_type.import_serialized(  # type: ignore
...
@pytest.mark.django_db
def test_update_local_jadawel_get_row_service(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_get_row_service_dispatch_transform(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table, fields, rows = data_fixture.build_table(
        user=user,
    ...
    dispatch_values = LocalJadawelUpsertRowServiceType().resolve_service_formulas(
...
@pytest.mark.django_db
def test_local_jadawel_get_row_service_dispatch_data_with_view_filter(data_fixture):
...
@pytest.mark.django_db
def test_local_jadawel_get_row_service_dispatch_data_with_service_search(
...
@pytest.mark.django_db
def test_local_jadawel_get_row_service_dispatch_data_with_service_integer_search(
    data_fixture, disable_full_text_search
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_get_row_service_dispatch_data_permission_denied(
    ...
    with (
        ...
        LocalJadawelGetRowUserServiceType().dispatch_data(
            service, {"table": table}, dispatch_context
        )


@pytest.mark.django_db
def test_local_jadawel_get_row_service_dispatch_validation_error(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table = data_fixture.create_database_table(user=user)
    integration = data_fixture.create_local_jadawel_integration(
...
@pytest.mark.django_db
def test_local_jadawel_get_row_service_dispatch_data_row_not_exist(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table = data_fixture.create_database_table(user=user)
    integration = data_fixture.create_local_jadawel_integration(
...
@pytest.mark.django_db
def test_local_jadawel_get_row_service_dispatch_data_no_row_id(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table, fields, rows = data_fixture.build_table(
        user=user,
...
@pytest.mark.django_db
def test_import_formula_local_jadawel_get_row_user_service_type(data_fixture):
    """
    Ensure that formulas are imported correctly when importing the
    LocalJadawelGetRowUserServiceType service type.

    The LocalJadawelGetRowUserServiceType::import_path() only supports 2 path
    parts, as opposed to LocalJadawelListRowsUserServiceType::import_path()
    which supports 3.
...
def test_local_jadawel_get_row_user_service_type_import_path(
    path, database_fields, expected
):
    """
    Ensure the LocalJadawelGetRowUserServiceType::import_path() correctly
...
def test_dispatch_transform_passes_field_ids(mock_get_serializer, field_names):
    """
    Test the LocalJadawelGetRowUserServiceType::dispatch_transform() method.
...
def test_extract_properties_with_empty_path_returns_all_fields(data_fixture):
    ...
    service = data_fixture.create_local_jadawel_get_row_service(table=table)
    service_type = LocalJadawelGetRowUserServiceType()
