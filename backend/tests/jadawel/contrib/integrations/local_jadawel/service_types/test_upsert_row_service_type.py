...
def test_local_jadawel_upsert_row_service_dispatch_data_without_row_id(
    data_fixture,
):
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_dispatch_data_with_row_id(
    data_fixture,
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_dispatch_data_disabled_field_mapping_fields(
    data_fixture,
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_dispatch_data_with_multiple_formulas(
    data_fixture,
):
    user = data_fixture.create_user()
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_dispatch_data_with_unknown_row_id(
    data_fixture,
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_dispatch_data_with_read_only_table_field(
    data_fixture,
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_dispatch_transform(
    data_fixture,
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_dispatch_data_incompatible_value(
    data_fixture,
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_dispatch_data_convert_value(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    integration = data_fixture.create_local_jadawel_integration(
        application=page.builder, user=user
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_resolve_service_formulas(
    data_fixture,
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_prepare_values(data_fixture):
    user = data_fixture.create_user()
    with pytest.raises(ValidationError) as exc:
        LocalJadawelUpsertRowServiceType().prepare_values(
...
def test_export_import_local_jadawel_upsert_row_service(
    data_fixture,
):
    ...
    get_row_service = LocalJadawelGetRow.objects.create(integration=integration)
    data_source = DataSourceService().create_data_source(
        user, service_type=get_row_service.get_type(), page=page
    )
    upsert_row_service = data_fixture.create_local_jadawel_upsert_row_service(
        integration=integration,
        table=table,
    ...
    imported_upsert_row_service = LocalJadawelUpsertRow.objects.get(
...
@pytest.mark.django_db()
def test_local_jadawel_upsert_row_service_after_update(data_fixture):
    user, token = data_fixture.create_user_and_token()
    page = data_fixture.create_builder_page(user=user)
    integration = data_fixture.create_local_jadawel_integration(
    ...
    field = data_fixture.create_text_field(table=table)
    LocalJadawelUpsertRowServiceType().after_update(
        service,
        {
            "table_id": table.id,
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_type_import_path(data_fixture):
    imported_upsert_row_service_type = LocalJadawelUpsertRowServiceType()
...
def test_dispatch_transform_passes_field_ids(
    ...
    """
    Test the LocalJadawelUpsertRowServiceType::dispatch_transform() method.
    ...
    service_type = LocalJadawelUpsertRowServiceType()
...
def test_extract_properties_returns_expected_list(path, expected):
    """
    Test the LocalJadawelUpsertRowServiceType::extract_properties() method.
...
@pytest.mark.django_db
def test_local_jadawel_upsert_row_service_dispatch_data_with_collaborators(
    data_fixture,
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
