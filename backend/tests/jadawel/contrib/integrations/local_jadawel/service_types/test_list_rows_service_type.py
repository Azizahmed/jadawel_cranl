...
from jadawel.contrib.database.api.rows.serializers import RowSerializer
...
from jadawel.contrib.database.views.models import SORT_ORDER_ASC, SORT_ORDER_DESC
from jadawel.contrib.database.views.view_filters import MultipleSelectHasViewFilterType
from jadawel.contrib.integrations.local_jadawel.models import LocalJadawelListRows
...
from jadawel.test_utils.helpers import AnyStr, setup_interesting_test_table
from jadawel.test_utils.pytest_conftest import FakeDispatchContext, fake_import_formula


@pytest.mark.django_db
def test_create_local_jadawel_list_rows_service(data_fixture):
...
@pytest.mark.django_db
def test_export_import_local_jadawel_list_rows_service(data_fixture):
    user = data_fixture.create_user()
    path_params = [
    ...
    service = data_fixture.create_local_jadawel_list_rows_service(
    ...
    service_sort = data_fixture.create_local_jadawel_table_service_sort(
    ...
    service: LocalJadawelListRows = service_type.import_serialized(  # type: ignore
...
@pytest.mark.django_db
def test_update_local_jadawel_list_rows_service(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    view = data_fixture.create_grid_view(user)
    integration = data_fixture.create_local_jadawel_integration(
...
@pytest.mark.django_db
def test_local_jadawel_list_rows_service_dispatch_transform(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table, fields, rows = data_fixture.build_table(
        user=user,
    ...
    service_type = LocalJadawelListRowsUserServiceType()
...
@pytest.mark.django_db
def test_local_jadawel_list_rows_service_dispatch_transform_with_default(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table, fields, rows = data_fixture.build_table(
        user=user,
    ...
    with override_settings(INTEGRATION_LOCAL_JADAWEL_PAGE_SIZE_LIMIT=30):
...
@pytest.mark.django_db
def test_local_jadawel_list_rows_service_dispatch_data_permission_denied(
    ...
    with (
        ...
        LocalJadawelListRowsUserServiceType().dispatch_data(
            service, {"table": table}, dispatch_context
        )


@pytest.mark.django_db
def test_local_jadawel_list_rows_service_before_dispatch_validation_error(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    integration = data_fixture.create_local_jadawel_integration(
        application=page.builder, user=user
    ...
    with pytest.raises(ServiceImproperlyConfiguredDispatchException):
        LocalJadawelListRowsUserServiceType().resolve_service_formulas(
            service, dispatch_context
        )


@pytest.mark.django_db
def test_local_jadawel_list_rows_service_dispatch_data_with_view_and_service_filters(
    data_fixture,
):
    user = data_fixture.create_user()
...
@pytest.mark.django_db
def test_local_jadawel_list_rows_service_dispatch_data_with_varying_filter_types(
    data_fixture,
):
    user = data_fixture.create_user()
    builder = data_fixture.create_builder_application(user=user)
    ...
    cost_150 = data_fixture.create_local_jadawel_table_service_filter(
...
@pytest.mark.django_db
def test_local_jadawel_list_rows_service_dispatch_data_with_view_and_service_sorts(
    data_fixture,
):
    user = data_fixture.create_user()
    builder = data_fixture.create_builder_application(user=user)
...
@pytest.mark.django_db
def test_local_jadawel_list_rows_service_dispatch_data_with_pagination(data_fixture):
    user = data_fixture.create_user()
    builder = data_fixture.create_builder_application(user=user)
    integration = data_fixture.create_local_jadawel_integration(
        application=builder, user=user
...
@pytest.mark.django_db
def test_local_jadawel_list_rows_service_import_context_path(data_fixture):
    local_jadawel_list_rows_service = LocalJadawelListRowsUserServiceType()

    assert local_jadawel_list_rows_service.import_context_path([], {}) == []
    assert local_jadawel_list_rows_service.import_context_path(["id"], {}) == ["id"]
    assert local_jadawel_list_rows_service.import_context_path(
...
@pytest.mark.django_db
def test_import_formula_local_jadawel_list_rows_user_service_type(data_fixture):
    """
    Ensure that formulas are imported correctly when importing the
    LocalJadawelListRowsUserServiceType service type.
    """
...
def test_local_jadawel_list_rows_user_service_type_import_path(
    path, database_fields, expected
):
    """
    Ensure the LocalJadawelListRowsUserServiceType::import_path() correctly
...
@pytest.mark.django_db
@patch("jadawel.contrib.integrations.local_jadawel.service_types.CoreHandler")
@pytest.mark.parametrize(
    "view_sorts",
    (
        [],
...
@patch(
    "jadawel.contrib.integrations.local_jadawel.service_types.get_row_serializer_class"
)
def test_dispatch_transform_passes_field_ids(mock_get_serializer, field_names):
    """
    Test the LocalJadawelListRowsUserServiceType::dispatch_transform() method.
