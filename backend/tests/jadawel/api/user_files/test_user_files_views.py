...
def test_upload_file_with_jwt_auth(api_client, data_fixture, tmpdir):
    ...
    with patch(
        "jadawel.core.user_files.handler.get_default_storage", new=lambda: storage
    ):
...
def test_upload_file_via_url_with_jwt_auth(api_client, data_fixture, tmpdir):
    ...
    responses.add(
        responses.GET,
        "https://jadawel.io/test2.txt",
        status=404,
    )
    response = api_client.post(
        reverse("api:user_files:upload_via_url"),
        data={"url": "https://jadawel.io/test2.txt"},
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    assert response.status_code == HTTP_400_BAD_REQUEST
    ...
    response = api_client.post(
        reverse("api:user_files:upload_via_url"),
        data={"url": "ftp://jadawel.io/test2.txt"},
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    assert response.status_code == HTTP_400_BAD_REQUEST
