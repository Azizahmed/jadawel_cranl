...
from jadawel.throttling.blacklist import (
    _token_key,
    blacklist_ip,
    blacklist_token,
...
from jadawel.throttling.middleware import ThrottleBlacklistMiddleware
...
@override_settings(JADAWEL_THROTTLE_BLACKLIST_TTL_SECONDS=7)
...
def test_blacklist_returns_remaining_ttl():
    with patch("jadawel.throttling.blacklist.time.time", return_value=100):
        blacklist_token("token-remaining", ttl=7)

    with patch("jadawel.throttling.blacklist.time.time", return_value=103.1):
...
def test_throttle_handler_import_path_is_valid():
    from jadawel.throttling.handler import ConcurrentUserRequestsThrottle
...
def test_throttle_populates_blacklist_on_deny(data_fixture):
    ...
    from jadawel.api.exceptions import ThrottledAPIException
    from jadawel.throttling.handler import ConcurrentUserRequestsThrottle
    ...
    # with the fixed cooldown (JADAWEL_THROTTLE_BLACKLIST_TTL_SECONDS).
...
@override_settings(JADAWEL_THROTTLE_BLACKLIST_TTL_SECONDS=9)
...
@override_settings(JADAWEL_THROTTLE_IP_ENABLED=True)
...
@override_settings(JADAWEL_THROTTLE_IP_ENABLED=False)
