...
from jadawel.api.settings.registries import SettingsDataRegistry, SettingsDataType
...
def test_register_settings_data_type(api_client, data_fixture):
    ...
    with patch("jadawel.api.settings.views.settings_data_registry", new=registry):
        response = api_client.get(reverse("api:settings:get"))
        assert response.status_code == HTTP_200_OK
        response_json = response.json()
