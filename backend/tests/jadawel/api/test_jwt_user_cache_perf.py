...
from jadawel.core.user.cache import invalidate_cached_user
...
def test_perf_cached_vs_uncached(data_fixture):
    ...
    with override_settings(JADAWEL_CACHE_TTL_SECONDS=30):
        invalidate_cached_user(user.id)
        avg_cached = _bench(auth, token)

    with override_settings(JADAWEL_CACHE_TTL_SECONDS=0):
        invalidate_cached_user(user.id)
