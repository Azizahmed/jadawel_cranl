...
show_help() {
    echo """
./dev.sh starts the Jadawel development environment and by default attempts to
...
dont_migrate    : Disable automatic database migration on Jadawel startup.
dont_sync       : Disable automatic template sync on Jadawel startup.
...
case "${1:-noneleft}" in
    ...
    ;;
    restart_wipe)
        echo "./dev.sh: Restarting Env and Wiping the pgdata volume if exists"
        shift
        down=true
        up=true
        delete_volumes=true
...
This will cause file permission errors when Jadawel starts up.

They are probably build files created by old Docker images owned by root.
